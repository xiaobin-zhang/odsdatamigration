...
class ValidationIntegrationTest {
    ...
    void fullValidationPassesOnH2() {
        ...
        assertTrue(saved.stream().allMatch(task -> task.getStatus() == TaskStatus.PASS));
        ProgressSnapshot progress = jobService.progress(batchId);
        assertEquals(100.0D, progress.getProgressPercent());
        assertEquals(9, progress.getTotalCount());
        assertEquals(9, progress.getPassCount());
    }
    ...
    void pauseAndResumeChangeBatchStatus() {
        ...
        assertEquals(9, jobService.progress(batchId).getTotalCount());
    }
